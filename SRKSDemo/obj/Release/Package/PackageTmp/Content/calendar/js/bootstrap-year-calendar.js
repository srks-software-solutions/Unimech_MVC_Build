 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_initializeEvents: function(opt) {
			...
			if(opt.holidaytype) { this.element.bind('holidaytype', opt.holidaytype); }
		...
		_renderHeader: function() {
            ...
            header.addClass('dispnone');//hiding the year slider pannel
			var headerTable = $(document.createElement('table'));
            
			var prevDiv = $(document.createElement('th'));
			prevDiv.addClass('prev');
            prevDiv.attr('onclick', 'updateDivsLeft();');
            ...
            prev2YearDiv.attr('onclick', 'getAllDetails(this.id)');
            prev2YearDiv.attr('id', this.options.startYear - 2);
            ...
            prevYearDiv.attr('onclick', 'getAllDetails(this.id)');
            prevYearDiv.attr('id', this.options.startYear - 1);
            ...
            yearDiv.addClass('currentYear');
            yearDiv.attr('onclick', 'getAllDetails(this.id)');
            yearDiv.attr('id', this.options.startYear);
            ...
            nextYearDiv.attr('onclick', 'getAllDetails(this.id)');
            nextYearDiv.attr('id', this.options.startYear + 1);
            ...
            next2YearDiv.attr('onclick', 'getAllDetails(this.id)');
            next2YearDiv.attr('id', this.options.startYear + 2);
            ...
            nextDiv.attr('onclick', 'updateDivsRight();');
			if(this.options.maxDate != null && this.options.maxDate < new Date(this.options.startYear + 1, 0, 1)) {
...

			{
				...
				case 'background':
					for(i in events){
						...
						if(events[i].location == 'Public Holiday'){
                            elt.parent().css('background-color', '#1784ea');
							elt.parent().css('color', '#fff');
						}else if(events[i].location == 'National Holiday'){
                            elt.parent().css('background-color', '#68ed10');
							elt.parent().css('color', '#fff');
                        } else if (events[i].location == 'Sunday') {
                            elt.parent().css('background-color', '#f21d09');
                            elt.parent().css('color', '#fff');
                        } else if (events[i].location == 'Saturday') {
                            elt.parent().css('background-color', '#f21d09');
                            elt.parent().css('color', '#fff');
                        }
					}
		...
		_applyEvents: function () {
			...
			cells.bind('holidaytype', function(e) {
				console.log(e);
			});
			/* Click right on date */
	...
	$.fn.holidaytype = function(fct) { $(this).bind('holidaytype', fct); }
